...
public class BucketheadZombie extends Zombie {
    public static final String AUDIO_HIT = "BucketheadHit";
    private static final String folderPath = "assets/Zombies/BucketheadZombie/";
    private final GameMenu menu;

    public BucketheadZombie(int level, Point location, GameMenu menu) {
        super(level, location);
        this.shape = new SpriteShape(new Dimension(this.width, this.height),
                new Dimension(60, this.height / 2),
                new Dimension(this.width / 4,
                        this.height / 3));

        ImageRenderer imageRenderer = new ZombieImageRenderer(this);
        List<ImageState> walkImageStates = imageStatesFromFolder(
                folderPath + "BucketheadZombie",
                imageRenderer);
        List<ImageState> attackImageStates = imageStatesFromFolder(
                folderPath + "BucketheadZombieAttack",
                imageRenderer);
    ...
    public void onDamaged(int damage) {
        ...
        if (this.HP == 0) {
            this.getWorld().addSprites(
                    new ZombieDie(new Point(this.location),
                            "assets/Zombies/NormalZombie/ZombieDie"));
            this.world = null;
        } else if (this.HP <= this.threshold) {
            this.getWorld().addSprites(new NormalZombie(
                    this.damage,
    ...
    public void onExploded(int damage) {
        ...
        if (this.world != null && this.HP == 0) {
            this.getWorld().addSprites(
                    new ZombieBoom(new Point(this.location)));
            this.world = null;
        }
    }

    public static class BucketheadZombieFactory implements ZombieFactory {
        @Override
        public Zombie getZombie(int level, Point location, GameMenu menu) {
            return new BucketheadZombie(level, location, menu);
        }
    }
}